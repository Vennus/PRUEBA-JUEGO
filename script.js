...
let textGanador = 
	document.getElementById("textGanador");
let botones = 
...
function ponerFicha(event){
	...
	if(!partidaAcabada && botonPulsado.innerHTML == ""){
		...
		if(estadoPartida == 1){
			textGanador.style.visibility = "visible";
			partidaAcabada = true;
		}
		else if(estadoPartida == -1){
			textGanador.innerHTML = "Has perdido"
			partidaAcabada = true;
			textGanador.style.visibility = "visible";
		}
	}	
...
function estado(){
	...
	function sonIguales(...args){
		...
		if(valores[0] != "" && valores.every((x, i, arr) => x===arr[0])){
			args.forEach(x => x.style.backgroundColor = "purple")
			return true;
		}
		else{
			return false;
